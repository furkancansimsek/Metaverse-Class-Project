...
public class NetworkedMarker : GrabbableEvents {
    ...
    IEnumerator WriteRoutine() {
        while (true) {
            if (Physics.Raycast(RaycastStart.position, RaycastStart.up, out RaycastHit hit, RaycastLength, DrawingLayers, QueryTriggerInteraction.Ignore)) {
                ...
                pv.RPC("InitDraw", RpcTarget.AllBuffered, drawStart, drawRotation, lineWidth, new float[]{DrawColor.r, DrawColor.g, DrawColor.b}, IsNewDraw );
            }
            else {
                IsNewDraw = true;                    
            }
    ...
    [PunRPC]
    public void InitDraw(Vector3 position, Quaternion rotation, float lineWidth, float[] _color, bool isdrawable) {
        if (isdrawable) {
            lastDrawPoint = position;
            DrawPoint(lastDrawPoint, position, lineWidth, (new Color(_color[0], _color[1], _color[2])), rotation, isdrawable);
            IsNewDraw = false;
        }
        else {
            float dist = Vector3.Distance(lastDrawPoint, position);
            if (dist > MinDrawDistance) {
                lastDrawPoint = DrawPoint(lastDrawPoint, position, lineWidth, DrawColor, rotation, isdrawable);
            }
        }
    }
    ...
    Vector3 DrawPoint(Vector3 lastDrawPoint, Vector3 endPosition, float lineWidth, Color lineColor, Quaternion rotation, bool isdrawable) {
        ...
        if (isdrawable || LineRenderer == null) {
